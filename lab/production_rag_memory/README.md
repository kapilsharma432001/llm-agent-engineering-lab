...
1. Authenticate and authorize the request and drive tenant_id, user_id, thread_id and permissions so retrieval is always tenant isolated.
...
4. Run required retrievals in parallel:-
    - Pinecone lon-term memory for relevant user memories
...
5. Fuse and rerank all retrieved evidence based on the relevance of standalone query, keeping only the strongest candidates.
6. Context builder - which combines recent conversation, relevant long-term memories, RAG chunks, and graph evidence while deduplicating and respecting the model's token budget.
    - If context is too large then summarise past conversations, remove low-rank evidences, or compress retrived chunks.
7. Send the final context to the LLM, then perform grounding/citation based before returning the response.
